...
bool hashmap_insert(HashMap *hashmap, char *key, void *value, void free_value(void* value), const bool to_free) {
  if (hashmap == NULL)
    return false;
  if (*hashmap == NULL)
    return false;
  ...
  *current = new_node;
  ...
  temp->s_elements = (*hashmap)->s_elements;

  for (uint64_t i = 0; i < old_s_buckets; i++) {
    head = &((*hashmap)->buckets[i]);
    if (head->key == NULL)
      continue;
